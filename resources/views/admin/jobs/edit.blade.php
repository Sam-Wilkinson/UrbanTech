...
@section('content')
<form method="POST" action="{{route('jobs.update',['job' => $job->id])}}" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    @if ($errors->any())
    ...
    <nav class="nav nav-pills nav-justified">
        <a class="nav-item nav-link active" id="nav-english-tab" data-toggle="tab" href="#nav-english" role="tab" aria-controls="nav-english" aria-selected="true">English</a>
        <a class="nav-item nav-link" id="nav-french-tab" data-toggle="tab" href="#nav-french" role="tab" aria-controls="nav-french" aria-selected="true">Francais</a>
        <a class="nav-item nav-link" id="nav-dutch-tab" data-toggle="tab" href="#nav-dutch" role="tab" aria-controls="nav-dutch" aria-selected="true">Nederlands</a>
    </nav>

    <div class="tab-content box" id="nav-tabContent">
        ...
        <div class="tab-pane fade show" id="nav-french" role="tabpanel" aria-labelledby="nav-french-tab">
            <div class="p-1 my-2">
                ...
                <!-- Categories -->
                    <div class="p-1 my-1">
                        ...
                        @foreach($categories as $category)
                            <div class="border bg-light p-3 mx-2">
                                <input type="radio" id="category" name="category" value="{{$category->id}}" 
                                {{old('category', $job->category? $job->category->id: ''==$category->id? 'checked' : '')}}><label for="coding">{{$category->name_fr}}</label>
                            </div>
                        @endforeach
                        <div class="border bg-light p-3 mx-2">
                            <a class="text-dark" href="{{route('categories.create')}}">Create a Category</a>
            ...
            <div class="tab-pane fade show" id="nav-dutch" role="tabpanel" aria-labelledby="nav-dutch-tab">
                <div class="p-1 my-2">
            ...
            <!-- Categories -->
                <div class="p-1 my-1">
                    ...
                    @foreach($categories as $category)
                        <div class="border bg-light p-3 mx-2">
                            <input type="radio" id="category" name="category" value="{{$category->id}}" 
                            {{old('category', $job->category? $job->category->id: ''==$category->id? 'checked' : '')}}><label for="coding">{{$category->name_nl}}</label>
                        </div>
                    @endforeach
                    <div class="border bg-light p-3 mx-2">
                        <a class="text-dark" href="{{route('categories.create')}}">Create a Category</a>
